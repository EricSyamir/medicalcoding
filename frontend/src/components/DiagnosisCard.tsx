...
import { ChevronDown, ChevronUp, Quote, Star, Pencil, Check, X } from "lucide-react";
import clsx from "clsx";
import type { DiagnosisCode } from "@/types/review";
...
export default function DiagnosisCard({ code, onOverride }: Props) {
  ...
  return (
    <div
      className={clsx(
        ...
        code.is_primary
          ? "border-navy-600 shadow-[0_0_0_2px_rgba(30,64,128,0.15)]"
    ...
    >
      ...
      <div className="p-4">
        <div className="flex items-start gap-3">
          {/* Code badge */}
          <span className="flex-shrink-0 font-mono text-xs font-bold px-2.5 py-1 rounded-lg bg-navy-800 text-white tracking-wide">
            {code.code}
          </span>

          {/* Primary star */}
          {code.is_primary && (
            <span className="flex-shrink-0 flex items-center gap-1 text-xs font-semibold text-amber-600 bg-amber-50 ring-1 ring-amber-200 px-2 py-0.5 rounded-full">
        ...
        <div className="mt-3 flex items-center gap-3">
          {code.evidence.length > 0 && (
            <button
              ...
              className="flex items-center gap-1 text-xs text-slate-500 hover:text-navy-700 transition-colors"
            >
